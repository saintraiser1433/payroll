  ...
  try {
    ...
    const salaryStats = salaryDistribution.map(dept => {
      const avgSalary = dept.employees.length > 0 
        ? dept.employees.reduce((sum, emp) => sum + (emp.salaryGrade?.salaryRate || 0), 0) / dept.employees.length
        : 0
      ...
      return {
        ...
        totalSalaryBudget: dept.employees.reduce((sum, emp) => sum + (emp.salaryGrade?.salaryRate || 0), 0)
      }
    })
